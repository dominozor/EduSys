function getCourseGradeOfStudent(userid, courseid) { //This function gets course grade data of a student from the Rest services of EduSys
    return $.ajax({
        type: "GET",
        url: "http://localhost:8090/rest/user/getExamGrade/"+userid+"/"+courseid,
...
function getAllGradesOfStudent(userid) { //This function gets all grades data of a student from the Rest services of EduSys
    return $.ajax({
        type: "GET",
        url: "http://localhost:8090/rest/user/getExamGrades/"+userid,
        async: false  // This option prevents this function to execute asynchronized
    });
}
...
function getAllExamsOfASection(courseid, sectionid) { //This function gets all exams of a section from the Rest services of EduSys
    return $.ajax({
        type: "GET",
        url: "http://localhost:8090/rest/course/getCourseSectionExams/"+courseid+"/"+sectionid,
        async: false  // This option prevents this function to execute asynchronized
    });
}
...
$(document).ready(function(){
    ...
    if(user["role"]===2) {
        ...
        $("#backToStudentPage").click(function(){
            ...
            window.location.replace("http://localhost:8090/templates/home/lecturer-home.html");
        });
        $('#addExam').click(function () {

            window.location.replace("http://localhost:8090/templates/exam/add-exam.html");
        });
